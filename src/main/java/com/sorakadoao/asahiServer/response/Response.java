...
public abstract class Response {
    ...
    public byte[] buildEncryptedPacket(){
        byte[] ans = null;
        try {
            byte[] encryptedContent = SM4Util.decrypt_ECB_Padding(responseInfo.connectionHandler.sm4key, build());
            byte[] rubbish = responseInfo.connectionHandler.generateRubbish(encryptedContent.length + 16);
            byte[] encryptedHeader = SM4Util.decrypt_ECB_Padding(responseInfo.connectionHandler.sm4key, responseInfo.buildResponseHeader(encryptedContent, rubbish.length));
